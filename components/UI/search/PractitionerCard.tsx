...
import type { PractitionerWithDetails } from "@/lib/practitioners";
...
export function PractitionerCard({
  ...
  const { firstName, lastName, specialty, city, price, convention, officialTags, communityTags, approvedAssos } =
    practitioner;
  ...
  return (
    <article
      className={`bg-white rounded-3xl border-2 p-5 transition-shadow ${
        isHighlighted ? "border-forest shadow-lg" : "border-forest/10 hover:shadow-md"
      }`}
    ...
    >
      ...
      <div className="flex items-start gap-4 mb-4">
        <div className="w-14 h-14 rounded-full bg-forest flex items-center justify-center shrink-0">
          <span className="text-cream font-heading font-bold text-lg">{initials}</span>
        </div>
        <div className="flex-1 min-w-0">
          ...
          {price && (
            <p className="text-forest/60 text-xs mt-0.5">
              ...
              {convention && (
                <span className="ml-1 text-forest/40">· {CONVENTION_LABELS[convention]}</span>
              )}
            </p>
          )}
        </div>
        ...
        <div className="bg-peach/30 border border-peach rounded-xl px-4 py-2.5 mb-4">
          <p className="text-forest text-sm font-body">
            ...
            <span className="font-semibold">
              {approvedAssos.map((a) => a.name).join(", ")}
            </span>
          </p>
        </div>
      )}
      ...
      {communityTags.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 mb-3">
          <span className="text-xs text-forest/40 font-body w-20 shrink-0">Communauté</span>
          <div className="flex flex-wrap gap-1.5">
      ...
      <div className="flex items-center justify-between pt-3 border-t border-forest/10 mt-3">
        <button
          ...
          aria-label={saved ? "Retirer des favoris" : "Sauvegarder ce praticien"}
        >
          {saved ? (
          ...
          <span className="font-body text-xs">{saved ? "Sauvegardé" : "Sauvegarder"}</span>
        </button>
        <Link
